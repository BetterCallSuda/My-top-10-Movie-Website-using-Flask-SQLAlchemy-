...
def add():
    ...
    if form.validate_on_submit():
        ...
        response = requests.get(MOVIE_DB_SEARCH_URL, params={"api_key": themoviedb_API_KEY, "query": movie_title})
...
def find():
    ...
    if movie_api_id:
        ...
        response = requests.get(movie_api_url, params={"api_key": themoviedb_API_KEY, "language": "en-US"})
        data = response.json()
        new_movie = Movie(
            title=data["title"],
...
class RateMovieForm(FlaskForm):
    ...
    ranking = StringField("Your ranking Out of 10")
